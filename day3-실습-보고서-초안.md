...
![애플리케이션 기동 및 정책 문서 적재 결과](<사진/01-앱기동-정책문서적재.jpeg>)

**그림 1. 애플리케이션 기동 및 정책 문서 적재 결과**
...
![Day 3 상담·대화 이력·환불 관리 API](<사진/02-swagger-day3-api.jpeg>)

**그림 2. Day 3 상담·대화 이력·환불 관리 API**
...
![RAG 정책 상담 응답과 return-policy 출처](<사진/03-rag-정책상담-출처.jpeg>)
...
![본인 주문 상태 조회 도구의 정상 실행](<사진/04-본인주문-상태조회.jpeg>)
...
![타 고객 주문 조회 차단](<사진/05-타고객주문-차단.jpeg>)
...
![동일 세션의 대화 맥락과 이력](<사진/06-멀티턴-세션이력.jpeg>)

![새 세션에서 주문번호를 다시 요청하는 세션 격리 결과](<사진/06-새세션-격리.jpeg>)
...
![환불 티켓 접수 결과](<사진/07-환불티켓-접수.jpeg>)

![환불 티켓 승인 대기 목록](<사진/07-환불티켓-대기목록.jpeg>)
...
![프롬프트 인젝션 차단 응답](<사진/08-프롬프트인젝션-차단.jpeg>)

**그림 8. 프롬프트 인젝션의 모델 호출 전 차단**

![감사 로그와 도구 호출 기록](<사진/09-감사로그-도구호출.jpeg>)

**그림 9. 감사 로그 및 개인정보 마스킹**

![AI 도구 호출 메트릭](<사진/10-ai-도구호출-메트릭.jpeg>)

**그림 10. 도구 호출 및 AI 처리 메트릭 확인**
...
![레드팀 시나리오 검증 결과](<사진/11-레드팀-검증결과.jpeg>)

**그림 11. 레드팀 시나리오 검증 결과**
...
| 9 | 레드팀 | 10개 시나리오 모두 방어하여 기준(8개 중 7개 이상)을 충족(그림 11) | 충족 |
